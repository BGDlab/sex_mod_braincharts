...
for (pheno in pheno_list) {
  ...
  mu_formula <- as.formula(
    paste0(pheno, " ~ ga(~ 10000 + s(sexMale_x_logAge, k=20, bs='ad') + ",
  ...
  saveRDS(test_mod, file = paste0("/mnt/isilon/bgdlab_processing/Margaret/sex_mod_braincharts/", pheno, "_test_mod2.rds"))
  
  fan_plot <- make_centile_fan(test_mod, df_final, x_var = "logAge_days", color_var = "sexMale")
  ggsave(filename = paste0("/mnt/isilon/bgdlab_processing/Margaret/sex_mod_braincharts/", pheno, "_test_centiles2.png"), fan_plot)
}

...
#SBATCH --array=1-4%10
#SBATCH --output=/mnt/isilon/bgdlab_processing/Margaret/sex_mod_braincharts/code/jobfiles/cv_tot_train/R-%A_%a.out
#SBATCH --error=/mnt/isilon/bgdlab_processing/Margaret/sex_mod_braincharts/code/jobfiles/cv_tot_train/R-%A_%a.err
...
WEIGHT=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $8}' $CONFIGFN )
SCALE=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $9}' $CONFIGFN )
...
Rscript $BASE/code/fit_cv_total_mods.R $DF $PHENO $LAMBDA $FS $TOTAL $SAVE_PATH $WEIGHT $SCALE

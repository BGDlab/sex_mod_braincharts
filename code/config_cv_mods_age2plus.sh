...
do
  ...
  echo "prepping: $split"
    ...
    if [[ "$rerun" == "TRUE" ]]; then
      date_tag=$(date +%Y%m%d)
      config_file=$config_path/cv_sample_${split}_total${total}_logAge${log_age}_age2plus_rerun${date_tag}_config.txt
    else
      config_file=$config_path/cv_sample_${split}_total${total}_logAge${log_age}_age2plus_config.txt
    fi
  ...
  for pheno_list in $(find $(realpath $pheno_lists) -type f -name "cortical_thickness.txt") #JUST RUNNING IN CT FOR NOW
  do
    ...
    #make output dir
      ...
      do
        ...
        if [[ "$rerun" == "TRUE" ]]; then
          best_file=$(ls ${save_path}/model_objs/${pheno_line}*_train_age2plus.rds 2>/dev/null | head -n 1 || true)
          if [[ -n "$best_file" ]]; then

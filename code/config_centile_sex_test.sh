...
do
  ...
  echo "prepping: $split"
    ...
    if [[ "$rerun" == "TRUE" ]]; then
      date_tag=$(date +%Y%m%d)
      config_file=$config_path/cv_sample_${split}_total${total}_logAge${log_age}_sexcenttest_rerun${date_tag}_config.txt
    else
      config_file=$config_path/cv_sample_${split}_total${total}_logAge${log_age}_sexcenttest_config.txt
    fi
    if ! [ -d $config_path ]
    then
      mkdir $config_path

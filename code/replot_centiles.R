...
if (isTRUE(total)){
  ...
  if (length(total_var) == 1){
    ...
    title_text <- paste0(pheno," ", tolower(traintest), "ing model controlling for total brain size (", total_var,") in Split-Half ", split)
    pt_show <- FALSE
  ...
  title_text <- paste0(pheno," ", tolower(traintest), "ing model in Split-Half ", split)
  pt_show <- TRUE
}

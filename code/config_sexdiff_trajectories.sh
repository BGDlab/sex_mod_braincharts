...
do
  ...
  echo "prepping: $split"
    ...
    if [[ "$rerun" == "TRUE" ]]; then
      date_tag=$(date +%Y%m%d)
      config_file=$config_path/cv_sample_${split}_total${total}_logAge${log_age}_sexdiff_rerun${date_tag}_config.txt
    else
      config_file=$config_path/cv_sample_${split}_total${total}_logAge${log_age}_sexdiff_config.txt
    fi
    if ! [ -d $config_path ]
    then
      mkdir $config_path
  ...
  do
    ...
    if [[ "$rerun" == "TRUE" ]]; then
        ...
        if [[ -n "$test_csv" ]]; then
          echo "Skipping $pheno_line (df found)"
          continue
        fi
    fi
	...
	# find testing model in same csv - handle optional _logPheno*_ in directory names
    mapfile -t matches < <(find "$save_path/model_objs" -type f -name "${pheno_line}_*test_full_mod.rds" 2>/dev/null)
    if [ ${#matches[@]} -eq 1 ]; then
      model="${matches[0]}"
          
      # Write the CSV file path and the formula to the output file (tab-delimited)
      echo -e "$model\t$file\t$save_path" >> "$config_file"
    elif [ ${#matches[@]} -eq 0 ]; then
      echo "Warning: No matching file found in '$save_path' for prefix '$pheno_line' and suffix 'test_full_mod.rds'" >&2
      #exit 1
    else
      echo "Warning: Multiple matching files found in '$save_path':" >&2
      printf '%s\n' "${matches[@]}" >&2
      #exit 1
    fi
  done < "$pheno_list"

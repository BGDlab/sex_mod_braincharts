...
do
  ...
  do
    ...
    config_file=$config_path/${filename}_${pheno_cat}_TEST_weight${1}_total${2}_config.txt
    if ! [ -d $config_path ]
    then
      mkdir $config_path
    elif [ -f $config_file ]
    ...
    #make output dir
      ...
      if [[ $2 == "TRUE" ]]; then
        ...
        # Write the CSV file path and the formula to the output file (tab-delimited)
          echo -e "$file\t$pheno_line\t$fs\t$tot\t$save_path" >> "$config_file"
        done < "$pheno_list"
        ...
        # Write the CSV file path and the formula to the output file (tab-delimited)
          echo -e "$file\t$pheno_line\t$fs\t$save_path" >> "$config_file"
